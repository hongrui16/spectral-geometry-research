...
- 有限步 KL 标度(`analysis_v2/kl_probe.py`,`slurm_v2/kl_probe.sbatch`):27 个跟踪矩阵 × 4 个单位范数方向(随机谱、纯缩放、frame 旋转、高斯)× 步长档位,32 条 GSM8K test 参考序列、3991 个答案 token。
  ...
  - ✅ 第二次(fp32 前向、TF32 关闭、步长 1e-4 到 3e-2,`results/v2/result_A/kl_probe_0.8b_fp32`,18 分钟):η∈[1e-3,1e-2] 上四个方向斜率均 2.01 [1.98, 2.15],局部二次成立;谱/frame 代价比逐矩阵 0.26–5.2、中位 0.95,q_proj 谱步便宜 3 倍、o_proj 谱步贵 2 倍;k_proj 缩放方向 KL≈0。数字见 `paper/results_draft.md` v2 登记处 A5,图 `figs/v2/figE_kl_probe.pdf`。
- **A5 完成。A 侧 v2 任务中不依赖 B 的项已全部完成。**
...
- 风险:(1) B 的 SFT 行全部低于 base,先要 P0.1 的回答;(2) ~~fp32 KL 探针若显示谱/frame 功能代价相同则立足点退到"SVD 基不特殊"~~ → 已排除:功能代价存在矩阵级 3–4 倍各向异性(q_proj 谱步便宜、o_proj 谱步贵),但无全局符号。因此论文不会坍缩成"SVD 基无关",而是"谱方向的学习价值(P3)与功能代价(A5)都要按矩阵报告,不存在范式级规律"。P3 的等 Frobenius 结果要用 A5 的代价表换算成等 KL 再裁决。
