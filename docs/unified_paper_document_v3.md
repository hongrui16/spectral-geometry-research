...
版本:v3 草案,2026-09-11(第三稿。第二稿以"双重分离"为主线,经复核 dense 基线为崩溃态后改写;见 §2.5)
...
## 3.2 局部二次模型下累计 KL 与字典无关(可检验预测)

参考分布 Fisher F 的局部二次模型下,T 步独立零均值步 δ_t 的累计 KL 期望为 ½Σ_t E[δ_tᵀFδ_t] = T × 单步 KL(交叉项为零)。
A5 表明单步单位范数 KL 在谱/frame 方向无全局差异 → 若步是随机游走,r 维与 mn 维更新在 T 步后的累计 KL 应相同。
**预测**:累计位移 ‖W_T−W_0‖_F 与累计 KL(W_0→W_T)相对 √T·单步值的比,反映相干漂移的强弱;健康区内各更新族的比值若相同,
则遗忘差异(若有)不能由相干漂移解释。E-v3-0a 在现有 ckpt 上测,批次二在 lr\* 上复测。
