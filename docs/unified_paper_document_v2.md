...
**当前状态（2026-09-09）。** H-A 至 H-D 尚无针对性数据。H-E 目前没有任何正面证据：同一方法家族的两个实现（§9.4 的 SSD 与自适应 α）在 0.8B 上分别为阴性与无效，PGSU 本身未实现。因此在阶段 C 完成之前，优化方法主张不进入论文主线；H-E 若届时仍无正面结果，按 §14 的阴性路径只保留离线诊断。
...
3. E0 中与已有数据直接相关的部分：矩形投影与维数基线、§5.4 的交叉样本 SNR 重算（替换 §9.2 的耦合指标）。
