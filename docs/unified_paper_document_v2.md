...
- **v1 主线**:credit assignment 决定梯度的 spectral/frame 几何(H1–H7)+ SSD 优化器。
- **成立**:捕获与指标管线;H2(跨优化器谱指标一致);A 侧 fp32 数值方案。
- **推翻**:H4 的 |C|–SNR 相关是同样本耦合产物;raw 对角能量的范式 ordering 在噪声主导区;"RLVR 保谱、SFT 改谱"未出现;SSD 无步长扫描、不能作方法证据。
  追加(09-11):B 机器 master 为 bf16,v1 B 侧全部训练结局作废。
- **主线是否还成立:不成立。** v2 保留测量对象与管线,核心问题改为条件谱增益。逐项见附录 A 末尾。
...
**v1 逐项判定(2026-09-09 定,09-11 追加):**

| | 结论 |
|---|---|
| 成立 | Phase 1 六格的捕获与指标管线可用;H2(跨优化器的谱指标一致)成立;fp32 master + bf16 autocast 的数值方案正确(A 侧)。 |
| 推翻 | H4 的 \|C\|–SNR 相关是同样本耦合产物(v2 §5、§9.3);raw 对角能量的范式 ordering 处于噪声主导区,不能作为机制证据;"RLVR 天然保谱、SFT 改谱"的范式预测在 B 的 Phase 2 上未出现;SSD 在 RLVR 上最差且未做步长扫描,不能作为方法证据。 |
| 追加(2026-09-11) | B 机器 master 权重实为 bf16(transformers 5.9.0 忽略 dtype),v1 B 侧全部训练结局(Phase 2、E1–E4、4B)作废,H 侧指标为量化噪声;A 侧 Phase 1 为 fp32,不受影响。 |
| 主线是否还成立 | **v1 主线不成立。** v2 保留其测量对象(奇异基下的更新几何)与数据管线,把核心问题改为"开放谱方向的条件增益"(§0.1)。v2 的后续状况见 `docs/unified_paper_document_v3.md` 开头。 |
