...
Usage: python scripts/summary_v2.py $RUNS/<run> [...more runs]"""
...
def summarize(run):
    ...
    if os.path.exists(log):
        ...
        sc = [(r["step"], round(r["scale_mean"], 2)) for r in steps
              if "scale_mean" in r]
        if sc:
            out["scale_curve"] = sc[::max(1, len(sc) // 20)]
    ...
    if os.path.exists(m):
        ...
        for c in ("G_r_spectrum", "H_r_spectrum", "r_null",
                  ...
                  "R_sigma_cross", "spearman_absC_snr_cross",
                  "spearman_absC_split", "snr_cross_diag_median"):
            if c in df:
                med[c] = float(df[c].median())
        if "r_null" in df.columns:
            med["R_enrich"] = float((df.G_r_spectrum / df.r_null).median())
            med["H_enrich"] = float((df.H_r_spectrum / df.r_null).median())
            if "R_sigma_cross" in df:
                med["R_sigma_cross_enrich"] = float(
                    (df.R_sigma_cross / df.r_null).median())
