...
from specgeom_v4.data import PromptSampler, build_prompt, load_gsm8k, reward_fn
from specgeom_v4.instrument import Instrumenter
from specgeom_v4.intervene_engine import InterventionEngine
from specgeom_v4.modeling import decoder_param_groups, load_model, stop_token_ids
from specgeom_v4.muon import Muon
...
def parse_args():
    ...
    p.add_argument("--intervention", default="none",
                   choices=["none", "spectrum_only", "frame_only",
                            ...
                            # v2 (docs/unified_paper_document_v4.md E4a):
...
class Trainer:
    def __init__(self, args):
        ...
        if args.optimizer == "adamw":
            ...
            import re as _re
            from specgeom_v4.ssd import SSD
    ...
    def run_evals(self, ck, step, final=False):
        ...
        for name, script, limit in (
            ...
            print(f"[eval] step {step} {name} -> {out} ({time.time() - t0:.0f}s)",
                  flush=True)
            if final:  # names expected by scripts_v4/summary.py
