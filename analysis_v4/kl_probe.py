"""E0/E2 (docs/unified_paper_document_v4.md §11 E2, §12.4): finite-step KL probe.
...
Usage (GPU):
  python analysis_v4/kl_probe.py --out results/v4/result_A/kl_probe_0.8b
...
from specgeom_v4.data import build_prompt, load_gsm8k
from specgeom_v4.instrument import select_tracked
...
@torch.no_grad()
def build_offtask_reference(tok, n_prompts, seed, device, max_len=512):
    """v4: off-task reference = MMLU *validation* items (question + choices, no answer),
    teacher-forced over all tokens after the first. Disjoint from the MMLU test split used
    for evaluation and from GSM8K. Same tensor layout as build_reference."""
    ...
    for i in idx:
        ...
        text = ex["question"].strip() + "\n" + "\n".join(
            f"{'ABCD'[k]}. {c}" for k, c in enumerate(ex["choices"])) + "\nAnswer:"
        seq = tok(text, add_special_tokens=False)["input_ids"][:max_len]
        ids.append(seq)
        masks.append([0] + [1] * (len(seq) - 1))
    T = max(len(s) for s in ids)
    pad = tok.pad_token_id
    input_ids = torch.full((len(ids), T), pad, dtype=torch.long)
    attn = torch.zeros((len(ids), T), dtype=torch.long)
...
def logp_batch(model, input_ids, attn, use_autocast):
    ...
    constant KL floor ~2e-4 nats/token (see results/v4/result_A/kl_probe_0.8b_bf16).
