...
Usage (GPU, ~1 min per checkpoint after the base is loaded):
  python analysis_v4/cum_kl.py --ckpts $RUNS/e4a_rlvr_full_s0/ckpt_000300 [...] \
      --out results/v4/result_B/cum_kl
  python analysis_v4/cum_kl.py --runs-root $RUNS --glob 'e4a_*' --step 300 --out ...
...
def main():
    ...
    ap.add_argument("--reference", choices=["task", "offtask"], default="task",
                    help="v4: task = GSM8K test solutions (v3 protocol); offtask = MMLU validation stems")
    a = ap.parse_args()
