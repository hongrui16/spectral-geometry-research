...
class Instrumenter:
    ...
    def capture_grad(self):
        ...
        for n, p in self.tracked.items():
            if p.grad is not None:
                # v3: G, H, W stored in fp32 (H = W_after - W_before must not be
                # re-quantized; the analysis SVD basis must match the engine's)
                self._record["mats"][n]["G"] = p.grad.detach().float().cpu().clone()
    ...
    def pre_optimizer(self):
        ...
        for n, p in self.tracked.items():
            self._record["mats"][n]["W"] = p.detach().float().cpu().clone()

    @torch.no_grad()
    def post_optimizer(self):
        for n, p in self.tracked.items():
            H = (p.detach() - self._w_before[n])
            self._record["mats"][n]["H"] = H.float().cpu().clone()
        self._w_before = None
