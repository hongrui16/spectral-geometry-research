...
每个 run 目录打包(共 10 个):
```bash
cd $RUNS && for d in phase2_*; do tar cf $d.tar $d/capture $d/log.jsonl $d/args.json $d/eval_gsm8k.json; done
```
传给作者A(网盘/scp/任何可访问路径均可)。**不要删 capture/**——里面的 G/H/W 捕获
是出 Fig.6/7 的原始数据;**不要改 analysis/ 下任何代码**,分析口径由作者A统一执行。
