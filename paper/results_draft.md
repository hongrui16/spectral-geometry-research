...
→ **在崩溃学习率上,保谱与去谱和 dense 一样崩**(MMLU 0.24,格式崩坏那一档),只有功能步长小两个量级的 r 维臂不崩。"保谱能防遗忘"在最需要它的地方也不成立;决定崩不崩的是功能步长,不是方向。RLVR lr×1 = 2e-6:exact_iso(修复版)s0/s1 GSM8K 0.594 / MMLU **0.222**、0.58 / 0.225(第 50 步已 0.25,格式崩,与 v2 e4a dense lr×1 的 0.24 相同);frame_matched s0/s1 GSM8K 0.542 / MMLU **0.240**、0.592 / 0.222(第 100 步已 0.22)。八个 lr×1 方向臂全部到齐。→ RLVR 上同样:保谱、钉谱都和 dense 一样崩,保谱的任务分数还更低。
...
| 配置 | GSM8K s0/s1(均值) | MMLU 1000 题 s0/s1(均值) | 任务外 KL | 任务 KL |
|---|---|---|---|---|
| dense(参照) | 0.408/0.422(0.415) | 0.462/0.450(0.456) | 0.026 | 0.85 |
| 倍率 alpha 1(o_proj 0.39、out_proj 0.58、gate 1.53、q 1.92) | 0.392/0.416(0.404) | 0.451/0.441(0.446) | 0.022(s1) | 0.85 |
| 倍率 alpha 2(o_proj 0.22、out_proj 0.29、gate 1.99、q 3.1) | 0.408/0.414(0.411) | 0.456/0.435(0.446) | 0.018 / 0.018 | 0.90 / 0.85 |

- 任务外 KL 按预期下降(alpha 1 −16%,alpha 2 −31%),任务 KL 与 GSM8K 不变。
- **但 MMLU(1000 题)没有变好,四个 run 均值 0.446 比 dense 低 0.010**(SE ≈ 0.011,方向反了,未显著)。任务外 KL 降三成没有换来能力保持——再次说明任务外 KL(MMLU 题干上的分布偏离)与 MMLU 准确率不是一回事。全量 MMLU(14042 题)在跑,以它为准;若全量也不高于 dense,方法在 SFT 上判负。
- 说明:q_proj 的倍率(1.9 / 3.1)来自只有 6 个矩阵的噪声比值,下一轮若继续应只按大类型(mlp 三个、注意力输出投影)给倍率。

**E-v4-4 RLVR(lr\* = 2e-7,倍率 `rlvr_type_a1.json`:o_proj 0.47、out_proj 0.57、k 0.45、gate 2.3、down 1.23、up 1.2)**

| 配置 | GSM8K s0/s1(均值) | MMLU 1000 题 | reward 末 100 步 | 任务外 KL | 任务 KL |
|---|---|---|---|---|---|
