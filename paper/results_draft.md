...
- 连同 lr 十倍(lr\* vs lr×1)不敏感:r 维干预在 lr×s_rel 跨 100 倍的范围内都停在同一点 (0.37–0.40, 0.46–0.48),该点与 dense lr×1/30 (0.390, 0.461) 重合;dense lr\* (0.415, 0.456) 在其右下方 1–2 SE,dense lr×1/3 则是"GSM8K 更高、MMLU 崩"的另一端。
- **H-3 裁决(SFT):前沿重合/交叉,r 维约束不在 dense 前沿右上方 → C5 阴性(SFT,终判)。** 写法:"逐步等范数下 r 维约束不优于调 lr;它只是把训练锁在 dense 的小步长点上,且对步长旋钮不响应"。
- 附带观察(进 §3.3 开放问题):r 维更新对 lr 和 s_rel 都不响应,说明其推进量不由等范数步长决定;与 E-v3-0c(功能步长小两个量级)一致,但"为何不随 s_rel 增大"未解释。
